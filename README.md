...
The demo seed command upserts `sku-dev-uaop8pff.myshopify.com`, replaces the repo's `demo-*` products for that shop, includes stable PDP component labels such as `product_description`, `shipping_returns`, and `recommendations`, and pre-generates diagnosis cards so `http://localhost:3000/?shop=sku-dev-uaop8pff.myshopify.com&window=24h` renders real board and product data immediately.
