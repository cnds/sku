...
- Priority-card selection should stay deterministic and score-driven; AI should explain the selected cards and produce the merchant-facing first action, not replace the ranking logic.
...
- The selected card's suspected friction and first fix should come from the same AI diagnosis chain used by product detail, with rule-based fallback copy when AI is unavailable.
...
- [x] Priority-card selection is driven by score-ranked product statistics, while AI is reserved for explaining selected cards and producing merchant-facing advice.
...
- [x] Priority-card suspected friction and first fix reuse the AI diagnosis chain when OpenAI-compatible credentials are configured, with local rule-based copy as fallback.
...
- [x] Product-detail AI diagnosis and priority-card advice share the same diagnosis sections so the card is the short version and the detail page explains the full logic and evidence.
...
- [x] Connect selected priority cards to the AI diagnosis chain so card-level first fixes are generated by the same evidence path as product-detail diagnosis.
